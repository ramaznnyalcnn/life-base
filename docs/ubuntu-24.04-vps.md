...
The frontend and mobile clients implement bearer-token login. Because `APP_API_TOKEN` is an additional shared service-token gate:

- create the first admin/user with `app.cli.create_user`
- leave `APP_API_TOKEN` empty for normal browser/mobile use
- enable `APP_API_TOKEN` only when every client can send the shared token through an outer auth layer or trusted service integration
...
Use `infra/nginx/lifeos.conf` as the base server block and adapt:
